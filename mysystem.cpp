#include "mysystem.h"
bool got_sleep=0;
int addcowv=0,addtigerv=0,addgrassv=0;
const int init_timer=1000/24;
...
void mysystem::mousePressEvent(QMouseEvent *event){
    if (addtigerv==1) tigerlist.insert(new Tiger(2000,event->localPos().x(),event->localPos().y(),rand()%2,cnt));
    if (addcowv==1) cowlist.insert(new Cow(2000,event->localPos().x(),event->localPos().y(),rand()%2,cnt));
    if (addgrassv==1) grasslist.insert(new Grass(1000,event->localPos().x(),event->localPos().y(),10,10,cnt));
}
